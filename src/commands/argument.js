	...
	async obtain(msg, val, promptLimit = Infinity) {
		...
		while(!valid || typeof valid === 'string') {
			...
			prompts.push(await msg.reply(stripIndents`
				${empty ? this.prompt : valid ? valid : `O argumento ${this.label} está inválido. Tente novamente.`}
				${oneLine`
		...
		while(true) { // eslint-disable-line no-constant-condition
			...
			while(!valid || typeof valid === 'string') {
				...
				if(val) {
					...
					prompts.push(await msg.reply(stripIndents`
						${valid ? valid : oneLine`
							O ${this.label} é inválido,
							"${escaped.length < 1850 ? escaped : '[muito longo]'}".
							Tente novamente.
						`}
						${oneLine`
							Responda com \`cancelar\` para cancelar o comando, ou \`finalizar\` para finalizar.
							${wait ? `O comando será cancelado automaticamente em ${this.wait} segundos.` : ''}
						`}
					`));
					...
					prompts.push(await msg.reply(stripIndents`
						...
						${oneLine`
							Responda com \`cancelar\` para cancelar o comando, ou \`finalizar\` para finalizar.
							${wait ? `O comando será cancelado automaticamente em ${this.wait} segundos, a não ser que você responda.` : ''}
						`}
					`));
				}
