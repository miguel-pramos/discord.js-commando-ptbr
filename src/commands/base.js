		...
		if (message.channel.type === 'text' && this.userPermissions) {
			...
			if (missing.length > 0) {
				if (missing.length === 1) {
					return `O comando \`${
						this.name
					}\` depende que você tenha a permissão "${
						permissions[missing[0]]
					}" .`;
				}
				return oneLine`
					O comando \`${this.name}\` depende que você tenha as seguintes permissões:
		...
		switch (reason) {
			case 'guildOnly':
				return message.reply(
					`O comando \`${this.name}\` deve ser usado em um canal de servidor.`
				);
			case 'nsfw':
				return message.reply(
					`O comando \`${this.name}\` só pode ser utilizado em canais NSFW.`
				);
			case 'permission': {
				if (data.response) return message.reply(data.response);
				return message.reply(
					`Você não tem permissão para usar o comando \`${this.name}\`.`
				);
			}
			case 'clientPermissions': {
				if (data.missing.length === 1) {
					return message.reply(
						...
						}" para que o comando \`${this.name}\` funcione.`
					);
				}
				return message.reply(oneLine`
					Preciso das seguintes permissions para que o comando \`${this.name}\` funcione:
					${data.missing.map((perm) => permissions[perm]).join(', ')}
				`);
			}
			case 'throttling': {
				return message.reply(
					`Você não deve usar o comando \`${
						this.name
					}\` novamente por ${data.remaining.toFixed(1)} segundos.`
				);
			}
			default:
				return null;
		...
		const ownerList = owners
			? owners
					.map((usr, i) => {
						...
						return `${or}${escapeMarkdown(usr.username)}#${
							usr.discriminator
						}`;
					})
